...
<<peds_study,echo=FALSE,results='hide',eval=TRUE>>=
suppressMessages(require(ggplot2))
suppressPackageStartupMessages(require(reshape2))
suppressMessages(require(ANTsR))
data(aal)
replot <- TRUE
demog <- read.csv('analysis/DemogWithASLVals.csv')
myrois <- c(1, 3, 7, 13, 37, 45, 55, 49, 63, 67,  81, 85, 87) # left precuneus
...
for (roi in myrois) {
  ...
  if(replot){
    ggplot(mydat.age.m, aes(Age, Value, colour=Measurement)) + geom_point() +
      geom_smooth(method="lm") + ggtitle(paste('Functional, Structural, PVC, and Raw CBF vs. Age for', aal$label_name[roi])) +
      xlab('Age (years)') + ylab('CBF (ml/100g/min)') + theme(text=element_text(size=20))
    ggsave(paste('figure/FunctionalStructuralDecomposition', aal$label_name[roi], '.pdf', sep=''),
      width=14, height=7)
    ggplot(mydat.thick.m, aes(Thickness, Value, colour=Measurement)) + geom_point() + 
      geom_smooth(method='lm') + xlab('Thickness (mm)') +
...
\begin{subfigure}{10cm}
  \includegraphics[width=10cm]{figure/FunctionalStructuralDecompositionHippocampus_L.pdf}
...
To demonstrate the utility and plausibility of the anatomically predicted and residual CBF in a population study of interest, we examined the dependence of the two signals as a function of age in a large pediatric dataset.  We found that the anatomically predicted and residual components of the raw CBF signal each had a lower variance than the original raw CBF image in most areas of the brain, and in some areas of the brain, such as the temporal pole, had opposite trends.  The trends were generally symmetric across the hemispheres.  For the left temporal pole, slope of the raw CBF vs.\ age was \Sexpr{peds.stat['Hippocampus_L', 'RawEst']}$\pm$\Sexpr{peds.stat['Hippocampus_L', 'RawStE']}, \textit{p}-value \Sexpr{peds.stat['Hippocampus_L', 'RawPVal']}; the corresponding structural slope was \Sexpr{peds.stat['Hippocampus_L', 'StructEst']}$\pm$\Sexpr{peds.stat['Hippocampus_L', 'StructStE']}, \textit{p}-value \Sexpr{peds.stat['Hippocampus_L', 'StructPVal']} and the functional slope was \Sexpr{peds.stat['Temporal_Pole_Mid_L', 'FuncEst']}$\pm$\Sexpr{peds.stat['Hippocampus_L', 'FuncStE']}, \textit{p}-value \Sexpr{peds.stat['Hippocampus_L', 'FuncPVal']}.  The structural CBF trended downwards with age, whereas the functional CBF trended upwards. In the precuneus, both structural and functional CBF trended downwards, resulting in an anatomically predicted CBF slope that was less pronounced than the raw CBF, whereas in lateral temporal lobe, structural CBF trended downwards while functional CBF stayed roughly constant. 
